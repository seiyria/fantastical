...
const dieRoll = require('../helpers/dieroll');

const human = require('./species').human;
...
module.exports.mysticOrder = () => {
    ...
    let name = sample(mysticOrder.patterns);
...
module.exports.militaryUnit = () => {
    ...
    let name = sample(militaryUnit.patterns);
...
module.exports.guild = () => {
    ...
    if(roll < 3) return `${capitalize(sample(guilds.roles))} of ${capitalize(sample(guilds.goals))}`;
    if(roll < 6) return `${capitalize(sample(guilds.adjectives))} ${capitalize(sample(guilds.actions))} ${capitalize(sample(guilds.titles))}`;
